...
export default function PublicNav({go,logged,role,switchRole,theme,setTheme}){
  return <header className="public-nav"><Logo/><nav><a className={location.hash==='#home'?'active':''} onClick={()=>go('home')}>Home</a><a className={location.hash==='#careers_dir'?'active':''} onClick={()=>go('careers_dir')}>Careers</a><a className={location.hash==='#jobs'?'active':''} onClick={()=>go('jobs')}>Jobs</a><a className={location.hash==='#how'?'active':''} onClick={()=>go('how')}>How It Works</a><a className={location.hash==='#about'?'active':''} onClick={()=>go('about')}>About</a></nav><div className="nav-actions">{logged&&<button className="role-btn" onClick={()=>switchRole(role==='student'?'employer':'student')}>{role==='student'?'Employer':'Student'} view</button>}<button className="ghost" onClick={()=>setTheme(theme==='light'?'dark':'light')}>{theme==='light'?'◐':'☀'}</button>{!logged&&<><Button variant="ghost" onClick={()=>go('login')}>Login</Button><Button onClick={()=>go('signup')}>Sign Up</Button></>}</div></header>
}
