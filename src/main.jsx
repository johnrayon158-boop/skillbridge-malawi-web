...
function App(){
  ...
  const pageContent=<>
    ...
    {page.startsWith('job-')&&<JobDetailPublic id={page.replace('job-','')}/>} 
...
function StudentDashboard({profile,go}){return <><PageHeader eyebrow="Student dashboard" title={`Welcome back, ${profile.name.split(' ')[0]}!`} description="Keep building your skills. Your future is bright." action={<Button onClick={()=>go('profile')}>Complete profile</Button>}/><div className="stats"><Stat icon="◉" label="Profile Completion" value={profile.completion+'%'} progress={profile.completion}/><Stat icon="✓" label="Skills Assessed" value="5"/><Stat icon="▤" label="Learning Resources" value="2"/><Stat icon="⌁" label="Career Matches" value="8"/></div><div className="dashboard-grid"><div className="panel"><PanelHead title="Recommended Careers" link="View all"/><div className="career-list">{careers.slice(0,4).map(c=><div className="career-item" key={c.title}><div className={'mini-icon '+c.color}>{c.icon}</div><div><b>{c.title}</b><span>{c.category}</span></div><strong>{c.match}% <small>match</small></strong><button onClick={()=>go('gap')}>View details →</button></div>)}</div></div><div className="panel quick"><PanelHead title="Quick Actions"/><Button variant="soft" onClick={()=>go('assessment')}>✓ Take Skills Assessment</Button><Button variant="soft" onClick={()=>go('careers')}>⌁ View Career Recommendations</Button><Button variant="soft" onClick={()=>go('learning')}>▤ Explore Learning Resources</Button><Button variant="soft" onClick={()=>go('profile')}>◉ Update Profile</Button></div></div><div className="panel"><PanelHead title="Recent jobs" link="View all"/><JobRows jobs={jobs.slice(0,3)} compact/></div></>}
...
function AssessmentRun({assessment,q,setQ,finish}){return <div className="assessment-run"><div className="assessment-top"><Badge tone="soft">{assessment.title}</Badge><span>Question {q} of 10</span></div><div className="question-card"><div className="q-progress"><i style={{width:q*10+'%'}}></i></div><h1>Which technology is commonly used to style the presentation of a web page?</h1><div className="options">{['JavaScript','HTML','CSS','SQL'].map((x,i)=><button key={x} className={i===2&&q===1?'chosen':''} onClick={()=>{}}><b>{String.fromCharCode(65+i)}</b>{x}</button>)}</div><div className="question-actions"><Button variant="outline" onClick={()=>setQ(Math.max(1,q-1))}>← Previous</Button>{q<10?<Button onClick={()=>setQ(q+1)}>Next →</Button>:<Button onClick={finish}>Submit Assessment</Button>}</div></div></div>}
...
function EmployerRows(){return <div className="job-rows">{['Blessings Chirwa','Chisomo Mbewe','Tapiwa Banda'].map((n,i)=><div className="job-row" key={n}><div className="avatar sm">{n.split(' ').map(x=>x[0]).join('')}</div><div><b>{n}</b><span>{['Frontend Developer','Data Analyst Intern','Graphic Designer'][i]}</span></div><Badge tone="success">{[91,86,79][i]}% match</Badge><Button variant="outline">Review</Button></div>)} </div>}
